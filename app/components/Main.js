...
var Main = React.createClass({
    ...
    playSong: function(result) {
        ...
        return axios.get("/spotify2/" + result.title)
            .then(function(response) {
                ...
                console.log("this is in main " + id);
            });
    },
    ...
    render: function() {
        ...
        var children = React.Children.map(this.props.children, function(child) {
            return React.cloneElement(child, { scrapedArticles: this.state.scrapedArticles, savedArticles: this.savedArticles, playSong: this.playSong, deletedArticle: this.deletedArticle, playlist: this.state.playlist }) }.bind(this))
        return (
            ...
            < nav >
            < div style = { nav } >
            < h2 > Good Tunes < /h2> < Link to = "/Scrape" > < button className = "btn btn-elegant" > Show Scrape < /button></Link >
            ...
            < h3 className = "panel-title text-center" > Scraped Playlist < /h3> < /div> < div className = "panel-body text-center" >

            { /* <Scrape scrape={this.state.scrapedArticles} savedArticles={this.savedArticles} />*/ }

            { /* YOU CAN INSERT THE NEXT CHILD HERE, POPULATING WITH THE API CALLS? */ } { children } { /*{React.cloneElement(this.props.children, {scrape: this.state.scrape})}*/ } {
                /*
